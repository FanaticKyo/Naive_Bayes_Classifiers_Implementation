...
class NaiveBayes(object):
    ...
    def estimateLogProbability(self, sentence):
        ...
        red = math.log(self.p_red)
        blue = math.log(self.p_blue)
        for token in token_list:
            if token in self.red_dict:
                red += math.log(self.red_dict[token])
            else:
                red += math.log(1 / self.red_denominator)

            if token in self.blue_dict:
                blue += math.log(self.blue_dict[token])
            else:
